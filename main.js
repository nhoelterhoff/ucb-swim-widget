...
if (modulePath != null) {
  ...
  console.log(
    "Failed to download new module and could not find any local version."
  );
}
...
async function downloadModule(scriptName, scriptUrl) {
  ...
  if (fm.fileExists(modulePath)) {
    ...
    if (moduleJs) {
      ...
      console.log(
        "Failed to download new module. Using latest local version: " +
          moduleLatestFile
      );
      return moduleLatestFile != null
        ? fm.joinPath(moduleDir, moduleLatestFile)
        : null;
    }
  }
}
